...
def validate_registry_client_sdk_distribution(ctx: ValidationContext, errors: list[str]) -> None:
    ...
    need(contract.get("schema_version") == "1.3", "SDK_DISTRIBUTION_SCHEMA_VERSION", errors)
    ...
    need(publish.get("tarball_integrity_evidence") == "REQUIRED", "SDK_DISTRIBUTION_TARBALL_EVIDENCE", errors)
    need(publish.get("tarball_integrity_match_required") is False, "SDK_DISTRIBUTION_TARBALL_MATCH_NOT_AUTHORITY", errors)
    need(publish.get("anonymous_remote_verification") == "REQUIRED", "SDK_DISTRIBUTION_ANONYMOUS_REMOTE", errors)
    content_verification = publish.get("content_verification", {})
    need(content_verification.get("method") == "EXACT_PACKAGE_CONTENT_SHA256_V1", "SDK_DISTRIBUTION_CONTENT_METHOD", errors)
    need(content_verification.get("manifest_format") == "npm-package-content-manifest/v1", "SDK_DISTRIBUTION_CONTENT_MANIFEST", errors)
    need(content_verification.get("path_set") == "EXACT", "SDK_DISTRIBUTION_CONTENT_PATH_SET", errors)
    need(content_verification.get("file_bytes_sha256") == "REQUIRED", "SDK_DISTRIBUTION_CONTENT_FILE_SHA256", errors)
    need(content_verification.get("file_mode") == "REQUIRED", "SDK_DISTRIBUTION_CONTENT_FILE_MODE", errors)
    need(content_verification.get("symlink_target") == "REQUIRED", "SDK_DISTRIBUTION_CONTENT_SYMLINK", errors)
    need(content_verification.get("remote_retrieval") == "ANONYMOUS", "SDK_DISTRIBUTION_CONTENT_ANONYMOUS", errors)
    need(content_verification.get("mismatch") == "FAIL_CLOSED", "SDK_DISTRIBUTION_CONTENT_FAIL_CLOSED", errors)
    existing_resolution = content_verification.get("existing_version_resolution", {})
    need(existing_resolution.get("primary") == "ANONYMOUS_CONTENT_FETCH", "SDK_DISTRIBUTION_EXISTING_PRIMARY", errors)
    need(existing_resolution.get("metadata_authority") == "FORBIDDEN", "SDK_DISTRIBUTION_METADATA_AUTHORITY", errors)
    need(existing_resolution.get("publish_conflict_recovery") == "EXACT_CONTENT_VERIFY", "SDK_DISTRIBUTION_CONFLICT_RECOVERY", errors)
    need(existing_resolution.get("publish_conflict_result") == "EXISTING_VERIFIED", "SDK_DISTRIBUTION_CONFLICT_RESULT", errors)
    registry_availability = content_verification.get("registry_availability", {})
    need(registry_availability.get("publish_time_security_scan_tolerant") is True, "SDK_DISTRIBUTION_SCAN_TOLERANT", errors)
    need(registry_availability.get("anonymous_retrieval_max_attempts") == 40, "SDK_DISTRIBUTION_SCAN_ATTEMPTS", errors)
    need(registry_availability.get("anonymous_retrieval_delay_seconds") == 30, "SDK_DISTRIBUTION_SCAN_DELAY", errors)
    need(registry_availability.get("timeout_result") == "FAIL_CLOSED", "SDK_DISTRIBUTION_SCAN_TIMEOUT", errors)
    ...
    if isinstance(workflow, str) and (ctx.root / workflow).is_file():
        ...
        need("EXACT_PACKAGE_CONTENT_SHA256_V1" in workflow_text, "SDK_WORKFLOW_CONTENT_METHOD", errors)
        need("npm-package-content-manifest/v1" in workflow_text, "SDK_WORKFLOW_CONTENT_MANIFEST", errors)
        need("local_content_digest" in workflow_text, "SDK_WORKFLOW_CONTENT_LOCAL_DIGEST", errors)
        need("remote_content_digest" in workflow_text, "SDK_WORKFLOW_CONTENT_REMOTE_DIGEST", errors)
        need("env -u NODE_AUTH_TOKEN NPM_CONFIG_USERCONFIG=/dev/null" in workflow_text, "SDK_WORKFLOW_ANONYMOUS_CONTENT_FETCH", errors)
        need("ANONYMOUS_CONTENT_FETCH" in workflow_text, "SDK_WORKFLOW_EXISTING_PRIMARY", errors)
        need("PUBLISH_CONFLICT_RECOVERED" in workflow_text, "SDK_WORKFLOW_CONFLICT_RECOVERY", errors)
        need("cannot publish over (the )?previously published versions" in workflow_text, "SDK_WORKFLOW_CONFLICT_PATTERN", errors)
        need("fetch_remote_package_with_retry 40 30" in workflow_text, "SDK_WORKFLOW_SCAN_RETRY_WINDOW", errors)
        need("publish-time security scanning" in workflow_text, "SDK_WORKFLOW_SCAN_REASON", errors)
        need("registry-security-scan wait" in workflow_text, "SDK_WORKFLOW_SCAN_FAIL_CLOSED", errors)
        need("registry-client-sdk-*-content-manifest.json" in workflow_text, "SDK_WORKFLOW_CONTENT_EVIDENCE", errors)
        need("Existing package integrity does not match local exact source package." not in workflow_text, "SDK_WORKFLOW_RAW_INTEGRITY_AUTHORITY_RETIRED", errors)
