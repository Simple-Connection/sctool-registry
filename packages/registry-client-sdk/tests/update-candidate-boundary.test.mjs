...
cases += 1;
equal(candidate.delivery.source, "cache", "candidate records actual verified source");
cases += 1;
equal(candidate.delivery.releaseId, 77, "candidate records exact release id");
cases += 1;
equal(candidate.delivery.assetId, 201, "candidate records exact asset id");
cases += 1;
equal(candidate.delivery.cacheFallbackUsed, false, "normal cache path has no fallback marker");
cases += 1;
